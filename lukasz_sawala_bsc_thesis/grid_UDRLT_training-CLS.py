...
BEST_MODEL_PATH = "CLS_best_bert_udrl.pth" 
STATE_DIM = 105
ACT_DIM = 8

"""
Best model (no action head): BATCH_SIZE=16, LEARNING_RATE=5e-05, EPOCHS=20
"""
...
def initiate_UDRLt_model() -> tuple:
    ...
    """
    # Inside initiate_UDRLt_model
    config = AutoConfig.from_pretrained("prajjwal1/bert-small")
    config.vocab_size = 1 # Still okay
    config.max_position_embeddings = 4 # Increase by 1 for CLS token
    model_bert = AutoModel.from_config(config).to(DEVICE)

    # Add CLS embedding
    cls_embedding = nn.Parameter(torch.randn(1, 1, config.hidden_size, device=DEVICE))  # (1, 1, hidden_size) for broadcasting
    ...
    # Return the CLS embedding as well
...
def train(learning_rate: float, epochs: int,
    ...
    """
    model_bert, d_r_encoder, d_h_encoder, state_encoder, head, cls_embedding = initiate_UDRLt_model()
    ...
    optimizer = optim.Adam(
        ...
        + [cls_embedding], # Add CLS embedding to optimizer
        lr=learning_rate,
    )
    loss_fn = nn.MSELoss()
        ...
        for s, r, h, a in train_loop:
            ...
            if scaler: # If using CUDA and AMP
                with torch.cuda.amp.autocast():
                    # Inside training/validation/evaluation loops
                    encoded_r = d_r_encoder(r).unsqueeze(1)
                    encoded_h = d_h_encoder(h).unsqueeze(1)
                    encoded_s = state_encoder(s).unsqueeze(1)

                    # Expand CLS embedding to batch size
                    batch_cls_embedding = cls_embedding.expand(s.size(0), -1, -1) # Shape: (batch_size, 1, hidden_size)

                    # Concatenate with CLS at the beginning
                    sequence = torch.cat([batch_cls_embedding, encoded_r, encoded_h, encoded_s], dim=1) # Shape: (batch_size, 4, hidden_size)

                    bert_out = model_bert(inputs_embeds=sequence).last_hidden_state # Shape: (batch_size, 4, hidden_size)

                    # Use the CLS token's output (at index 0)
                    pred = head(bert_out[:, 0]) # Shape: (batch_size, hidden_size) -> (batch_size, ACT_DIM)
